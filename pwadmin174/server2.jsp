...
<%
	...
	{
		out.println("<p align=\"right\" style=\"color:#b71c1c;\"><b>" + T("server.login_required") + "</b></p>");
	}
	else
	{
		allowed = true;
...
<head>
    ...
    <style>
        ...
        th, td {
             padding: 8px;
            text-align: left;
             white-space: normal; /* Changed to normal */
       ...
       .server-box tr {
          background-color: #333; /*Match table and container background*/
         }
         .status-frame {
            width: 150px; /* Increased frame width */
           height: 40px; /* Increased frame height */
           }
    </style>
</head>
<body>
...
<td align="center" valign="top">
	...
	<tr>
		<td colspan="2" align="left" valign="middle" style="border-bottom: 1px solid #cccccc;">
			<iframe src="status.jsp" class="status-frame" scrolling="no" frameborder="no" style="border: 0;"></iframe>
		</td>
	</tr>
	<tr>
		<td>
  ...
  <tr>
    <td colspan="2" style="border-top: 1px solid #cccccc;">
       Rebuilt by Crucifix, Creez,Krona and corzca
    </td>
  </tr>
	</table>
</div>
...
<%
	if(allowed)
	{
		...
		// ### Show Form ###
		 out.println("<div class='server-box'>");
        out.println("<form action='index.jsp?page=server&process=save' method='post' style='margin: 0px;'>");
        out.println("<input type='hidden' name='serverpage' value='" + request.getParameter("serverpage") + "'>");
			out.println("<table cellpadding='2' cellspacing='0'>");
			out.println("<tr><th colspan='3' align='center' style='padding: 5px;'><font color='#ffffff'>SERVER CONFIGURATION</font></th>");
			out.println("<tr bgcolor='#f0f0f0'><td colspan='3' align='center'>" + message + "</td>");
			out.println("<tr><td style='border-top: 1px solid #cccccc;'>Max. Online Users / Virtual Max. Online Users</td><td align='center' style='border-top: 1px solid #cccccc;'><input name='users_max' type='text' value='" + users_max + "' style='width: 40px; text-align: center;' class='server-input'></td></tr>");
			out.println("<tr><td style='border-top: 1px solid #cccccc;'>Set Lambda Value</td><td align='center' style='border-top: 1px solid #cccccc;'><input name='lambda_value' type='text' value='" + lambda + "' style='width: 40px; text-align: center;' class='server-input'></td></tr>");
			...
			out.println("<tr><td colspan='2' align='center' style='border-top: 1px solid #cccccc;'><input type='image' src='include/btn_save.jpg' style='border: 0px;' class='server-button'></td></tr>");
			out.println("</table>");
			out.println("</form>");
        out.println("</div>");
...
<td align="center" valign="top">
    ...
    <tr>
    <td align="center" style="border-top: 1px solid #cccccc;">
	<form action="index.jsp?page=server&serverpage=<%=request.getParameter("serverpage")%>&process=exp" name="form_doubleexp" method="post">
	<input type="hidden" name="doubleexp" value=""/>
	<label>EXP / SP Rate:
		<select name="expselect"  class="server-select">
    ...
    <tr>
    <td align="center" style="border-top: 1px solid #cccccc;">
	<input align="middle" type="image" src="include/btn_save.jpg" name="doubleexp_button" style="border: 0px;" class="server-button" onclick="javascript:f_doubleexp_submit()">
	</form>
	</td>
	<tr>
